...
> bstInsert k v (Bind k' v' l r) = list_to_tree $ add_to_list k v $ toBinds (Bind k' v' l r)
...
> delete_from_list :: (Ord k) => k -> [(k, v)] -> [(k, v)]
> delete_from_list _ [] = []
> delete_from_list k (x:xs)
>           | k == (fst x) = xs
>           | k < (fst x) = x : xs
...
> bstDelete _ Emp = Emp
> bstDelete k (Bind k' v' l r) = list_to_tree $ delete_from_list k $ toBinds (Bind k' v' l r)
